...
from app.database.models import EnvironmentEntity
...
class EnvironmentRepository(
    BaseRepository[EnvironmentEntity, EnvironmentCreate, EnvironmentUpdate, EnvironmentRead]
):
    """Persistence boundary for environment profile CRUD operations."""

    def __init__(self, session: AsyncSession) -> None:
        super().__init__(session=session, entity_type=EnvironmentEntity, read_schema=EnvironmentRead)

    async def create(self, payload: EnvironmentCreate) -> EnvironmentRead:
        entity = EnvironmentEntity(**self._payload_dict(payload))
    ...
    async def get_by_id(self, environment_id: UUID) -> EnvironmentRead:
        entity = await self.session.get(self.entity_type, environment_id)
        if entity is None:
            raise EntityNotFoundError("EnvironmentEntity", environment_id)
        return self._entity_to_read_model(entity, self.read_schema)

    async def list_all(self) -> list[EnvironmentRead]:
    ...
    async def list_paginated(self, query: EnvironmentListQuery) -> PaginatedResponse[EnvironmentRead]:
    ...
    async def update(self, environment_id: UUID, payload: EnvironmentUpdate) -> EnvironmentRead:
        entity = await self.session.get(self.entity_type, environment_id)
        if entity is None:
            raise EntityNotFoundError("EnvironmentEntity", environment_id)
