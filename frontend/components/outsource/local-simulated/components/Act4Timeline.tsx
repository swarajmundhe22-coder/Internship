...
const Act4Timeline = ({ result, narrative, onNext }: Props) => {
  ...
  const riskScoreBand = result.uncertaintyBands?.riskScore;
  const lifespanBand = result.uncertaintyBands?.predictedLifespan;

  const exportCsv = (fileName: string, header: string[], row: Array<string | number>) => {
  ...
  const downloadRiskCiCsv = () => {
    exportCsv(
      ...
      [
        ...
        riskScoreBand?.lower ?? "",
        riskScoreBand?.upper ?? "",
        riskScoreBand?.confidenceLevel ?? "",
      ]
    );
  };

  const downloadLifespanCiCsv = () => {
    exportCsv(
      "lifespan-ci-overlay.csv",
      [
        "metric",
        "remaining_lifespan_ci_lower_years",
        "remaining_lifespan_ci_upper_years",
        "confidence_level",
      ],
      [
        "predicted_lifespan",
      ...
      <div className="relative z-10 w-full max-w-7xl h-full flex flex-col gap-6 md:gap-8 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8 min-h-0">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="glass p-6 rounded-none border border-white/5 space-y-4">
              <p className="text-[9px] font-display font-bold uppercase tracking-widest text-white/60">
                Region Pack: {result.regionName || "Unknown"}
              </p>
              <div className="space-y-2">
                <p className="text-[9px] font-display font-bold uppercase tracking-widest text-white/50">Risk Score CI</p>
                <p className="text-[11px] text-white/80 font-mono">
                  {riskScoreBand?.lower ?? "-"} to {riskScoreBand?.upper ?? "-"} ({riskScoreBand?.confidenceLevel ?? "-"})
                </p>
              </div>
              <div className="space-y-2">
                <p className="text-[9px] font-display font-bold uppercase tracking-widest text-white/50">Lifespan CI</p>
                <p className="text-[11px] text-white/80 font-mono">
                  {lifespanBand?.lower ?? "-"} to {lifespanBand?.upper ?? "-"} ({lifespanBand?.confidenceLevel ?? "-"})
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={downloadRiskCiCsv}
                  className="px-3 py-2 text-[10px] font-display font-bold uppercase tracking-widest border border-accent/40 text-accent hover:border-accent transition-all"
                >
                  Risk CI CSV
                </button>
                <button
                  type="button"
                  onClick={downloadLifespanCiCsv}
                  className="px-3 py-2 text-[10px] font-display font-bold uppercase tracking-widest border border-white/20 text-white/80 hover:border-white/40 transition-all"
                >
                  Lifespan CI CSV
                </button>
              </div>
